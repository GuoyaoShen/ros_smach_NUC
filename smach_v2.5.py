    ...
    def execute(self, userdata):
        ...
        if self._flag_human == 1:
            ...
            if self._flagg == 0:
                self._time_last = rospy.get_time()
            self._flagg = 1
            if self._time - self._time_last > 10:
        ...
        if self._humans.human_body != []:
            ...
            if (1 in self._part_ls and 4 in self._part_ls):
                ...
                if neck[1] > hand[1]:
                    ...
                    return 'default'
            elif (14 not in self._part_ls and 15 not in self._part_ls ) and (16 in self._part_ls and 17 in self._part_ls):
                return('turn_away')
...
class HumanNum(smach.State):
    ...
    def execute(self, userdata):
        if self._humans.human_body != []:
            ...
            if 8 > self._numHuman > 3:
                rospy.loginfo('HumanNum MANY')
                return 'many'

            elif self._numHuman > 7:
                rospy.loginfo('HumanNum TOO MANY')
                return 'many_shy'
            ...
            if len(self._depthHistory) > 15:
                ...
                if len(self._depthHistory) > 1:
                    if all([self._depthHistory[i] < self._depthHistory[i+1] for i in range(len(self._depthHistory)-1)]):
                        ...
                        if (self._depthHistory[0]-self._depthHistory[-1]) > 5000:
...
class HumanHeight(smach.State):
    ...
    def execute(self, userdata):
        ...
        if self._humans.human_body != []:
            ...
            if  self._depth < 20000 and 0 in self._part_ls:
                nose_ind = self._part_ls.index(0)

                if self._closest_humanbody.joint_y[nose_ind] > 300:
                    return 'short'
                else:
                    return 'tall'
        return 'default'
...
class UF_SayBye(smach.State):
    ...
    def execute(self, userdata):
        ...
        rospy.loginfo('TOPICS_SAY_BYE published')
        return 'sent_UF_SayBye'
...
class UF_Surprise(smach.State):
    def __init__(self):
        ...
        self.time_init1 = rospy.get_time()
    ...
    def execute(self, userdata):
        ...
        if time - self.time_init1 > 60:
            self.time_init1 = time
...
class UF_Shy(smach.State):
    def __init__(self):
        smach.State.__init__(self,
                             outcomes=['sent_UF_Shy'], )
    ...
    def execute(self, userdata):
        ...
        if time - self.time_init > 360:
            self.time_init = time
            self.pub.publish('shy')

        rospy.loginfo('TOPICS_SHY published')
        return 'sent_UF_Shy'
    ...
    with sm_top:
        ...
        sm_con = smach.Concurrence(outcomes=['default', 'say_hello', 'greeting', 'greeting_child', 'say_bye',
                                   ...
                                   outcome_map={'say_hello': {'HUMAN_DISTANCE': 'far', 'FACE_POSE': 'straight'},
                                                ...
                                                'mimic_face': {'HUMAN_NUM': 'default', 'HUMAN_DISTANCE': 'close', 'FACE_POSE': 'straight',
                                                               'HUMAN_HEIGHT': 'tall'},
# 
                                                'surprise': {'HUMAN_WALKING': 'walking_close_fast'},
                                                'dance': {'HUMAN_NUM': 'many'},
                                                'shy': {'HUMAN_NUM': 'many_shy'}})
        ...
        smach.StateMachine.add('UF_SHY', UF_Shy(), transitions={'sent_UF_Shy': 'INITCON'})
